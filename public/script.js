(function () {
    ...
    window.addEventListener('load', function () {
        ...
        function go() {
            ...
            console.log('Shuffling enabled:', enableShuffling);
            ...
            api.sessionexists(id, function (exists) {
                ...
                if (!exists) return setError('Session does not exist. Try deleting or generating a new session');
            
                api.editsession(id, httpproxy, enableShuffling, function () {
                    editSession(id, httpproxy, enableShuffling);
                    
                    if (enableShuffling) {
                        api.shuffleDict(id, function(dict) {
                            var shuffler = new StrShuffler(dict); // Use server's dictionary
                            var finalUrl = shuffler.shuffle(url);
                            console.log('Shuffled URL:', finalUrl);
                            console.log('Redirecting to: /' + id + '/' + finalUrl);
                            // redirect
                            window.location.href = '/' + id + '/' + finalUrl;
                        });
                    } else {
                        console.log('Redirecting to: /' + id + '/' + url);
